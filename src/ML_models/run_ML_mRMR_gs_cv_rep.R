#!/bin/Rscript
###############################################################################
# ML models using a grid search hyperparameter tuning (repeated 10-fold c-v) #
###############################################################################
## Generate ML results 
## command: Rscript run_ML_mRMR_gs_cv_rep.R inputset (Rscript run_ML_mRMR_gs_cv_rep.R data_driven)
...
if ("cv_rep"%in%list.files("../../ML_out/two_class") == FALSE){
  dir.create("../../ML_out/two_class/cv_rep", showWarnings = FALSE)}
if ("copd_genes"%in%list.files("../../ML_out/two_class/cv_rep") == FALSE){
  dir.create("../../ML_out/two_class/cv_rep/copd_genes", showWarnings = FALSE)}
if ("mrmr_dea"%in%list.files("../../ML_out/two_class/cv_rep/") == FALSE){
    dir.create("../../ML_out/two_class/cv_rep/mrmr_dea", showWarnings = FALSE)}
if ("mrmr"%in%list.files("../../ML_out/two_class/cv_rep/") == FALSE){
      dir.create("../../ML_out/two_class/cv_rep/mrmr", showWarnings = FALSE)}
...
if(class(a) == "numeric"){
  ...
  for( i in seq(a, a+15, by=5)){
    ...
    save(results, file=paste("../../ML_out/two_class/cv_rep/mrmr/results_",i,".Rda", sep= ""))
  }
}else{
    ...
    print(results)
    # Save as a Rdata file
    save(results, file= paste('../../ML_out/two_class/cv_rep/',a,'/results.Rda', sep = ""))
}
