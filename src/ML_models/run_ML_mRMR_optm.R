...
if(a == "all_genes"){
  ...
  genes <- scan("../../Inputs_lists/data_driven.txt", what = "character", sep = ",")
  copd_ctrl_train <- copd_ctrl_train[,colnames(copd_ctrl_train) %in%
                                       c("dis_condition",genes)]
}else if(a == "data_driven_guildify"){
  genes <- scan("../../Inputs_lists/data_driven_guildify.txt", what = "character", sep = ",")
  copd_ctrl_train <- copd_ctrl_train[,colnames(copd_ctrl_train) %in%
                                       c("dis_condition",genes)]
}else if(a == "disgenet"){
  genes <- scan("../../Inputs_lists/disgenet_curated.txt", what = "character", sep = ",")
  copd_ctrl_train <- copd_ctrl_train[,colnames(copd_ctrl_train) %in%
                                       c("dis_condition",genes)]
}else if(a == "disgenet_curated_guildify"){
  genes <- scan("../../Inputs_lists/disgenet_curated_guildify.txt", what = "character", sep = ",")
  copd_ctrl_train <- copd_ctrl_train[,colnames(copd_ctrl_train) %in%
                                       c("dis_condition",genes)]
}else if(a == "disgenet_entire_list"){
  genes <- scan("../../Inputs_lists/disgenet_entire_list.txt", what = "character", sep = ",")
  copd_ctrl_train <- copd_ctrl_train[,colnames(copd_ctrl_train) %in%
                                       c("dis_condition",genes)]
}else{
  a = as.numeric(a)
...
# Metrics
auc_acc_sens_spec <- metric_set(mcc,bal_accuracy, accuracy, yardstick::sens,  yardstick::spec,
                                precision, roc_auc, mn_log_loss)
acc_sens_spec <- metric_set(mcc,bal_accuracy, accuracy, yardstick::sens, yardstick::spec, precision)
...
ML_models <- function(copd_ctrl_train_mrmr){
  ...
  best_bal_accuracy_rf <- select_best(rf_res, "bal_accuracy") # Best based on accuracy
  ...
  best_bal_accuracy_svm_r <- select_best(svm_r_res, "bal_accuracy") # Best based on accuracy
  ...
  best_bal_accuracy_svm_p <- select_best(svm_p_res, "bal_accuracy") # Best based on accuracy
  ...
  best_bal_accuracy_en <- select_best(en_res, "bal_accuracy") # Best based on accuracy
  ...
  best_bal_accuracy_knn <- select_best(knn_res, "bal_accuracy") # Best based on accuracy
  ...
  best_bal_accuracy_xgb <- select_best(xgb_res, "bal_accuracy") # Best based on accuracy
...
#  collect_parameters(copd_ctrl_model_st, "svm_r_res") %>% filter(coef != 0)
  ...
  for(i in 1:length(classifiers)){
          ...
          loss_auc_train_copd <-  pred_train %>%
              group_by(fold) %>%
          ...
          metrics_train_copd <- all_metrics_copd %>%
           ...
           all_metrics_ctrl <- pred_train %>%
               group_by(fold) %>%
               conf_mat(observation, prediction) %>%
               mutate(summary_tbl = lapply(conf_mat, summary, event_level = "second")) %>%
               unnest(summary_tbl)

           metrics_train_ctrl <- all_metrics_ctrl %>%
               group_by(.metric) %>%
               summarise(
                   mean = mean(.estimate, na.rm = TRUE),
                   sd = sd(.estimate, na.rm = TRUE)
            ...
            ## Macro F1
            ## Save f1 for each category
            copd_f1 <- all_metrics_copd %>% filter(.metric == "f_meas")
            ctrl_f1 <- all_metrics_ctrl %>% filter(.metric == "f_meas")
            copd_ctrl_f1 <- rbind(copd_f1, ctrl_f1)
            ## Compute the macro F1 for each resample
            macro_F1_by_fold <- copd_ctrl_f1 %>% group_by(fold) %>%
              summarise(.estimate = mean(.estimate, na.rm = TRUE))
            ## Compute the mean of all resamples
            macro_F1 <- macro_F1_by_fold %>% summarise(
              mean = mean(.estimate, na.rm = TRUE),
              sd = sd(.estimate, na.rm = TRUE))
            ## Add it to metrics_train_copd and metrics_train_ctrl
            macro_F1 <- data.frame(.metric = "macro_F1", mean = macro_F1$mean, sd = macro_F1$sd)
            metrics_train_copd <- rbind(metrics_train_copd, macro_F1)
            metrics_train_ctrl <- rbind(metrics_train_ctrl, macro_F1)


          metrics_train_copd <- rbind(metrics_train_copd, loss_auc_train_copd)
          matrix_train <- conf_mat_resampled(classifier)
          r_train[[i]] <- list("metrics_copd" = metrics_train_copd,
                               "metrics_ctrl" = metrics_train_ctrl,
                               "matrix" = matrix_train)
  ...
  for(i in 1:length(fits)){
          ...
          metrics_test_copd <- summary(matrix_test)

          metrics_test_ctrl <- summary(matrix_test, event_level = "second")
          ...
          metrics_test_copd <- rbind(metrics_test_copd, loss_auc_test)

          ## Compute the macro F1
          macro_F1 <- rbind(metrics_test_copd, metrics_test_ctrl) %>%
                      filter(.metric == "f_meas") %>%
                       summarise(
                          .estimate = mean(.estimate, na.rm = TRUE))
          ## Add it to metrics_train_copd and metrics_train_ctrl
          macro_F1 <- data.frame(.metric = "macro_F1", .estimator = "binary", .estimate = macro_F1$.estimate)
          metrics_test_copd <- rbind(metrics_test_copd, macro_F1)
          metrics_test_ctrl <- rbind(metrics_test_ctrl, macro_F1)
          ...
          r_all_train[[i]] <- list("metrics" = metrics_all_train,
                                   "matrix" = matrix_all_train)
          s_all_train[[i]] <- misclassified_samples_all_train
          r_test[[i]] <- list("metrics_copd" = metrics_test_copd,
                              "metrics_ctrl" = metrics_test_ctrl,
                              "matrix" = matrix_test)
