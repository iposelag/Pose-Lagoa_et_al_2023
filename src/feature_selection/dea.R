...
###############################################################################
## command: Rscript dea.R esetobject pvalue logfoldchange (eset 0.01 1.5)
...
if ("DEA"%in%list.files("../../data/") == FALSE){
  dir.create("../../data/DEA/", showWarnings = FALSE)}
# we will adjust by platform and sex
if ("platform_sex"%in%list.files("../../data/DEA/") == FALSE){
  dir.create("../../data/DEA/platform_sex", showWarnings = FALSE)}
...
if(esetobject == "eset_several_class"){
  ...
  groups <- levels(eset$dis_condition)
  comb <- c("COPD - CTRL")
  myDEA2(eset, groups, comb, pv = pv, lfc = log2(lfc) )
}
