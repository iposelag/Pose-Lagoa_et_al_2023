...
if ("cv_rep"%in%list.files("../../ML_out/three_class") == FALSE){
  dir.create("../../ML_out/three_class/cv_rep", showWarnings = FALSE)}
if ("dea"%in%list.files("../../ML_out/three_class/cv_rep") == FALSE){
  dir.create("../../ML_out/three_class/cv_rep/dea", showWarnings = FALSE)}
if ("copd_genes"%in%list.files("../../ML_out/three_class/cv_rep") == FALSE){
  dir.create("../../ML_out/three_class/cv_rep/copd_genes", showWarnings = FALSE)}
if ("mrmr"%in%list.files("../../ML_out/three_class/cv_rep/") == FALSE){
    dir.create("../../ML_out/three_class/cv_rep/mrmr", showWarnings = FALSE)}
  ...
  xgb_rs <- xgb_fit %>%
    ...
    for(i in 1:length(classifiers)){
            ...
            missclasified_samples_train <- missclasified_samples_train %>%
                                            rename(
                                              row_id = Var1)
            missclasified_samples_train <- as.data.frame(merge(missclasified_samples_train, sample_identifier, by = "row_id") %>%
                                              group_by(row_id) %>%
...
if(class(a) == "numeric"){
  ...
  for( i in seq(a, a+5, by=5)){
    ...
    save(results, file=paste("../../ML_out/three_class/cv_rep/mrmr/results_",i,".Rda", sep= ""))
    ...
    print(results)
    # Save as a Rdata file
    save(results, file= paste('../../ML_out/three_class/cv_rep/',a,'/results.Rda', sep = ""))
}
